...
//   BWP 0 URLLC:  100 MHz @ 6.0 GHz (UMa)          → ~500 Mbps peak DL
//   BWP 1 eMBB:   100 MHz @ 3.5 GHz (UMa)          → ~500 Mbps peak DL
//   BWP 2 mMTC:   10  MHz @ 0.7 GHz (RMa)          → ~30  Mbps peak DL
// Adjust the PEAK_MBPS array if you change BW or propagation scenarios.
// ---------------------------------------------------------------------------
static constexpr double PEAK_MBPS[NUM_SLICES] = {50.0,
                                                 500.0,
                                                 30.0}; // IMPROVEMENT: eMBB 500 Mbps for 100 MHz
...
SliceGymEnv::SliceGymEnv()
{
    ...
    m_sla[SLICE_URLLC] = {.minThroughputMbps = 1.0, .maxDelayMs = 1.0, .maxPrbUtil = 0.85};

    m_sla[SLICE_EMBB] = {.minThroughputMbps = 20.0, .maxDelayMs = 50.0, .maxPrbUtil = 0.90};

    m_sla[SLICE_MMTC] = {.minThroughputMbps = 0.1, .maxDelayMs = 200.0, .maxPrbUtil = 0.80};
...
void
SliceGymEnv::RegisterSliceSubnet(uint8_t sliceId, Ipv4Address network, Ipv4Mask mask)
{
    NS_ASSERT_MSG(sliceId < NUM_SLICES, "Slice ID out of range");
...
bool
SliceGymEnv::IsInSlice(uint8_t sliceId, Ipv4Address /*src*/, Ipv4Address dst) const
{
...
SliceGymEnv::GetObservationSpace()
{
    ...
    {
        low[i * 3 + 0] = 0.0f;
        high[i * 3 + 0] = 1000.0f; // throughput [Mbps]
        low[i * 3 + 1] = 0.0f;
        high[i * 3 + 1] = 1000.0f; // delay [ms]
        low[i * 3 + 2] = 0.0f;
        high[i * 3 + 2] = 1.0f; // prb_util [0,1]
    }
...
{
    std::vector<uint32_t> shape = {OBS_SIZE};
    Ptr<OpenGymBoxContainer<float>> obs = CreateObject<OpenGymBoxContainer<float>>(shape);
...
SliceGymEnv::GetReward()
{
    ...
    for (uint8_t s = 0; s < NUM_SLICES; ++s)
    {
        ...
        {
            sliceViol +=
                (kpi_s.prbUtilization - sla_s.maxPrbUtil) / (1.0 - sla_s.maxPrbUtil + 1e-6);
        }
...
SliceGymEnv::GetExtraInfo()
{
    ...
    for (uint8_t s = 0; s < NUM_SLICES; ++s)
    {
        oss << names[s] << "[tput=" << std::fixed << std::setprecision(2)
            << m_kpis[s].throughputMbps << "Mbps,dly=" << m_kpis[s].avgDelayMs << "ms"
